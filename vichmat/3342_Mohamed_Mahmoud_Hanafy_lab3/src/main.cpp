...
double bisect(double a, double b, double eps, int &k, double delta)
{
    double e = fabs(eps) * 2.0; // Tolerance for the interval
    double fa = f(a, delta);    // f(a)
    double fb = f(b, delta);    // f(b)
    double x = 0.5 * (a + b);   // Midpoint of the interval
    double fx;
    ...
    for(k = 0; b - a >= e; k++) // Iterate until the interval is smaller than the tolerance
    {
        x = 0.5 * (a + b); // Calculate the midpoint
        fx = f(x, delta);  // Evaluate f(x)

        if (fx == 0.0) // If the midpoint is the root
        {
            return x;
        }
...
void test_table()
{
    ...
    for (double delta = 0.1; delta >= 0.000001; delta /= 10) // Vary delta
    {
        ...
        {
            x = bisect(a, b, eps, k, delta); // Find the root using the bisection method
            double n_m = eps / delta;         // Ratio of eps to delta
            double n = 1 / f1(x);             // Inverse of the derivative f'(x)
            int k_t = (int)round(log2((b - a) / eps)); // Theoretical number of iterations
            int C = (eps / delta > n) ? 1 : 0; // Condition C

            printf("%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%d\t%d\t%d\n",
                   eps, delta, a, b, x, n_m, n, k, k_t, C);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    double result = bisect(a, b, eps, k, delta);
